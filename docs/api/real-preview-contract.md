...
`numeric_source_coordinate` means a valid source coordinate pair exists. Coordinates with `source-provided` or `source-precision-unknown` precision are presented as approximate and remain pending human review; they are never described as exact. The observed aggregate includes 486 Italy groups with a `(0,0)` source pair; the importer reports this anomaly, and those values remain pending review rather than being called exact. `city_postal` means the selected source group has city or postal placement and no usable coordinate pair; it has no point coordinates. Rows or groups without usable placement remain private observations and are excluded from candidate routes. Identifier syntax alone is never used to infer location or candidate class.
