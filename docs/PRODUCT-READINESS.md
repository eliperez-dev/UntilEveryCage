...
| Frontend product | In progress | V2 is a developer preview using fixtures/local synthetic data; V1 vanilla JavaScript remains public. The subordinate [frontend design backlog](frontend/roadmap.md) records the approved direction, deferred ideas, and current contract gaps. | Approve information architecture and visual direction, then build a production Svelte frontend; close the linked contract gaps in this tracker before enabling unsupported record families. |
