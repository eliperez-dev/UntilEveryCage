...
pub async fn queue(
    ...
    let sql = match kind.as_str() {
        "contradictions" => {
            "SELECT claim_id, facility_id, organization_id, claim_domain, claim_kind, observed_at, confidence FROM uec.claim_current c WHERE c.review_state IN ('disputed','review_required') ORDER BY observed_at DESC, claim_id DESC LIMIT $1"
        }
        "unresolved-identities" => {
            "SELECT crosswalk_id, left_identifier_id, right_identifier_id, assertion_status, confidence, observed_at FROM uec.source_entity_crosswalks WHERE assertion_status IN ('candidate','review_required','disputed') ORDER BY observed_at DESC, crosswalk_id DESC LIMIT $1"
        }
        "quarantine" => {
            "SELECT source_record_id, source_id, source_state, received_at FROM uec.source_records WHERE source_state IN ('quarantined','rejected') ORDER BY received_at DESC, source_record_id DESC LIMIT $1"
        }
        _ => {
            "SELECT 'claims' AS metric, count(*)::bigint AS value FROM uec.claims UNION ALL SELECT 'relationships', count(*) FROM uec.organization_relationship_observations UNION ALL SELECT 'quarantined_records', count(*) FROM uec.source_records WHERE source_state IN ('quarantined','rejected')"
        }
    };
