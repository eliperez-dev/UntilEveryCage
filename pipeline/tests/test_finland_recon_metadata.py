...
FINLAND_IDS = {
    "fi.ruokavirasto.approved-food", "fi.ruokavirasto.feed-abp", "fi.luke.agriculture",
    "fi.aquaculture", "fi.animal-experiments", "fi.syke.environment",
    "fi.prh.ytj.organizations", "fi.statfin.pxweb",
}

class FinlandReconMetadataTests(unittest.TestCase):
    def test_sources_are_registered_and_document_is_row_free(self):
        ...
        self.assertTrue(FINLAND_IDS.issubset({s["source_id"] for s in registry["sources"]}))
        doc = (ROOT / "docs/country-recon-fi.md").read_text(encoding="utf-8")
