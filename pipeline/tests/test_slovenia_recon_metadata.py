...
class SloveniaReconMetadataTests(unittest.TestCase):
 def test_row_free_sources(self):
  r=json.loads((ROOT/"pipeline/source_registry.json").read_text()); self.assertTrue(IDS.issubset({x["source_id"] for x in r["sources"]})); d=(ROOT/"docs/country-recon-si.md").read_text(); self.assertIn("row-free",d); self.assertIn("fully automated",d)
