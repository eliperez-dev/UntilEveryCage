...
```powershell
python -m pipeline.refresh_private --source dk.smiley --mode fixture --artifact dk.smiley=fixture.csv
...
deterministic plan. Retry count is bounded to five. The plan and manifests are
aggregate-only and must not contain rows or source payloads.
