#!/usr/bin/env python3
"""Record an explicit, append-only restoration of public access."""
import argparse, os, uuid
import psycopg

DEFAULT_DB = "postgresql://uec:uec-local-development-only@localhost:5433/uec"

def restore(database_url, source_record_id, policy_version, maintainer, note=None):
    with psycopg.connect(database_url) as connection:
        with connection.transaction():
            connection.execute("""
                ...
                VALUES (%s, %s, 'public_access_restored', 'other', %s, %s, %s)
            """, (uuid.uuid4(), source_record_id, policy_version, maintainer, note))
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    restore(a.database_url, a.source_record_id, a.policy_version, a.maintainer, a.note)
    print("Recorded explicit public-access restoration; evidence was not modified.")
