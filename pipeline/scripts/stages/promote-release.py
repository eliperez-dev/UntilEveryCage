...
def promote(database_url: str, release_id: str) -> dict:
    with psycopg.connect(database_url) as connection:
        with connection.transaction():
            target = connection.execute("SELECT status FROM uec.releases WHERE release_id = %s FOR UPDATE", (release_id,)).fetchone()
            if not target:
            ...
            previous = connection.execute("SELECT release_id FROM uec.releases WHERE status = 'promoted' AND release_id <> %s", (release_id,)).fetchall()
            connection.execute("UPDATE uec.releases SET status = 'validated' WHERE status = 'promoted' AND release_id <> %s", (release_id,))
