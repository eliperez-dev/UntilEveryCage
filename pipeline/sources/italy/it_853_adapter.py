...
class Italy853Adapter:
 source_id="it.853-2004"; adapter_version="it-853-candidate-v1"; schema_version="it-853-csv-v2.0"
 def parse_bytes(self,content):
  digest=hashlib.sha256(content).hexdigest(); rows=list(csv.DictReader(content.decode("utf-8-sig").splitlines(),delimiter=";"))
  if not rows or tuple(rows[0])!=REQUIRED: raise ValueError("schema drift")
  seen=set(); occurrences={}; accepted=[]; quarantined=[]
  for line,row in enumerate(rows,2):
   rec=clean(row.get(REQUIRED[1])); act=clean(row.get("codice_impianto_attivita")); key=(rec,act); reasons=[]; occurrences[key]=occurrences.get(key,0)+1
   if None in row or None in row.values() or any(isinstance(v, list) for v in row.values()): reasons.append("malformed_row_shape")
   if not rec: reasons.append("missing_recognition_number")
   if not act: reasons.append("missing_activity_code")
   # Repeated recognition/activity rows are quarantined instead of silently collapsed.
   if key in seen: reasons.append("ambiguous_repeated_recognition_activity")
   seen.add(key); status=clean(row.get("stato_attivita"))
   if status and status.upper() not in STATUS: reasons.append("unknown_status")
   status=STATUS.get(status.upper()) if status else None
   # Sensitive source_values are retained only in private parsed output; normalized exposure is screened.
   out={"source_id":self.source_id,"source_row":line,"source_row_id":row_id(row, occurrences[key]),"source_values":dict(row),"normalized":{"recognition_number":rec,"facility_grouping":"provisional-recognition-number","name":clean(row.get("ragione_sociale")),"address":None,"municipality":clean(row.get("comune")),"region":clean(row.get("regione")),"activity_code":act,"activity_description":clean(row.get("descrizione_impianto_attivita")),"products":clean(row.get("prodotti_abilitati")),"status":status,"coordinates":None,"privacy_gate":"pending-review","publication_gate":"blocked"}}
   (quarantined if reasons else accepted).append({"reasons":tuple(reasons),"record":out} if reasons else out)
 ...
 def run(self, raw_path, run_dir, artifact):
  raw=Path(raw_path).read_bytes(); result=self.parse_bytes(raw)
  if artifact.sha256 != result["source_sha256"] or artifact.byte_size != len(raw): raise ValueError("artifact provenance mismatch")
  root=Path(run_dir); (root/"normalized").mkdir(parents=True,exist_ok=True); (root/"quarantined").mkdir(parents=True,exist_ok=True)
