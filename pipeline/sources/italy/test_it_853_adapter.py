...
class Test(unittest.TestCase):
 ...
 def test_sensitive_and_deterministic_identity(self):
        content=(H+"\n"+row()).encode(); a=Italy853Adapter(); x=a.parse_bytes(content)["accepted"][0]; y=a.parse_bytes(content)["accepted"][0]; self.assertEqual(x["source_row_id"],y["source_row_id"]); self.assertNotIn("p_iva",x["normalized"]); self.assertEqual(x["normalized"]["coordinates"]["precision"],"source-precision-unknown")

 def test_invalid_coordinate_pairs_and_ranges_are_quarantined(self):
  a=Italy853Adapter()
  for bad_row in (row().replace(";12;45;", ";12;;"), row().replace(";12;45;", ";200;45;"), row().replace(";12;45;", ";north;45;")):
   result=a.parse_bytes((H+"\n"+bad_row).encode())
