"""Run one DGAL section through private acquisition or assisted capture."""
...
def refresh(*, section: str, run_dir: str | Path, raw_path: str | Path | None = None, fetch: bool = False, terms_review_path: str | Path | None = None, output_root: str | Path = "data/raw", run_id: str | None = None, retrieved_at_utc: str | None = None, timeout_seconds: float = 60.0, max_bytes: int = 32 * 1024 * 1024) -> dict[str, Any]:
    ...
    if fetch:
        ...
        metadata = fetch_section(section=section, output_root=output_root, terms_review_path=terms_review_path, run_id=run_id, timeout_seconds=timeout_seconds, max_bytes=max_bytes)
    ...
    if lifecycle.get("status") == "candidate-ready":
        ...
        write_operator_review_packet(run_root, lifecycle["manifest"], source_scope=lifecycle["manifest"]["coverage"], checks=("review candidate-handoff/records.jsonl in restricted staging", "confirm no public release or API promotion", "review source terms and privacy before approval"), blockers=("candidate is private and human-gated", "address and coordinate publication blocked"))
    report = {"source_id": adapter.source_id, "section": section, "source_url": artifact.source_url, "retrieved_at_utc": artifact.retrieved_at_utc, "checksum_sha256": artifact.sha256, "byte_size": artifact.byte_size, "lifecycle_status": lifecycle.get("status"), "run_dir": lifecycle.get("run_dir"), "publication_state": lifecycle.get("publication_state", "unchanged"), "release_state": "not-created", "geocoding": "disabled"}
    atomic_json(root / "refresh.json", report); return {"report": report, "lifecycle": lifecycle}
