<script lang="ts">
  ...
  const loadLocal = async (cursor?: string, append = false) => {
    ...
    if (!append) { invalidateDetail(); localStatus = 'loading'; localFailure = 'unknown'; localError = ''; pagingError = ''; manifestSha256 = undefined; loaded = []; selected = undefined; nextCursor = null; coverageNote = ''; coverageScope = ''; countSemantics = ''; } else { paging = true; pagingError = ''; }
    try {
