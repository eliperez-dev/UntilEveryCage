<script lang="ts">
  ...
  // V2 deliberately has no q parameter. Search never refetches a page.
  $: remoteQuery = `${apiProfile}|${region}|${category}|${sourceType}|${displayPrecision}|${lifecycleStatus}`;
  ...
  const reviewLabel = (location: Location): string => location.evidence?.factualReviewStatus === 'unreviewed' ? 'Factually unreviewed' : 'Factually reviewed';
  ...
  const loadLocal = async (cursor?: string, append = false) => {
    ...
    const queryKey = `${requestProfile}|${region}|${category}|${sourceType}|${displayPrecision}|${lifecycleStatus}`;
    ...
    if (!append) { invalidateDetail(); localStatus = 'loading'; localFailure = 'unknown'; localError = ''; manifestSha256 = undefined; loaded = []; selected = undefined; nextCursor = null; coverageNote = ''; coverageScope = ''; countSemantics = ''; } else paging = true;
    try {
      const result = await repo.list(requestProfile, { country_code: region === 'all' ? undefined : region, category: category === 'all' ? undefined : category, source_type: sourceType === 'all' ? undefined : sourceType, display_precision: displayPrecision === 'all' ? undefined : displayPrecision, lifecycle_status: lifecycleStatus === 'all' ? undefined : lifecycleStatus, cursor }, controller.signal);
      if (generation !== listGeneration) return;
      loaded = append ? [...loaded, ...result.locations] : result.locations;
      if (!selected) selected = result.locations[0];
      release = result.releaseId; ruleset = result.ruleset; nextCursor = result.nextCursor; coverageNote = result.coverageNote; coverageScope = result.coverageScope ?? ''; countSemantics = result.countSemantics ?? ''; localStatus = 'ready'; localFailure = 'unknown'; paging = false;
      if (!append) await syncRoute();
    } catch (error) {
      paging = false; if (!append) activeListKey = ''; if (generation !== listGeneration) return;
      const kind = error && typeof error === 'object' && 'kind' in error ? (error as { kind: ApiError['kind'] }).kind : 'unknown';
      if (kind === 'aborted') return;
      localStatus = kind === 'no-release' ? 'no-release' : 'error'; localFailure = kind; localError = error instanceof Error ? error.message : 'The V2 list response was rejected safely.'; loaded = []; selected = undefined;
    }
  };
  ...
  onMount(() => {
    ...
    if (localMode) {
      ...
      try { const api = params.get('api') ?? undefined; repo = new LocalLocationRepository(globalThis.fetch, api); csvRepo = new LocalCsvExportRepository(globalThis.fetch, api ?? ''); metadataStatus = 'loading'; void new FilterMetadataRepository(globalThis.fetch, api ?? '').get().then((value) => { metadata = value; metadataStatus = 'ready'; }).catch(() => { metadataStatus = 'error'; }); }
    ...
    window.addEventListener('hashchange', onHashChange); window.addEventListener('popstate', onPopState); return () => { listAbort?.abort(); detailAbort?.abort(); window.removeEventListener('hashchange', onHashChange); window.removeEventListener('popstate', onPopState); };
  });
</script>
...
<main class:dev-preview={devPreviewMode}>
  ...
  {#if localMode && search.trim()}<p class="search-context" role="status">Text search is limited to the currently loaded V2 page; the public contract does not provide a server-side free-text query. {nextCursor ? 'Later pages may contain additional matches.' : 'All rows in this response are loaded.'}</p>{/if}
