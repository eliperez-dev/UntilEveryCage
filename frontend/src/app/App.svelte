<script lang="ts">
  ...
  const loadLocal = async (cursor?: string, append = false) => {
    ...
    if (!append) { invalidateDetail(); localStatus = 'loading'; localError = ''; manifestSha256 = undefined; loaded = []; selected = undefined; nextCursor = null; coverageNote = ''; } else paging = true;
    try { const result = await repo.list(profile === 'community' ? 'community' : 'official', { country_code: region === 'all' ? undefined : region, category: category === 'all' ? undefined : category, source_type: sourceType === 'all' ? undefined : sourceType, display_precision: displayPrecision === 'all' ? undefined : displayPrecision, lifecycle_status: lifecycleStatus === 'all' ? undefined : lifecycleStatus, cursor }, controller.signal); if (generation !== listGeneration) return; loaded = append ? [...loaded, ...result.locations] : result.locations; if (!selected) selected = result.locations[0]; release = result.releaseId; ruleset = result.ruleset; nextCursor = result.nextCursor; coverageNote = result.coverageNote; localStatus = 'ready'; paging = false; if (!append) await syncRoute(); }
