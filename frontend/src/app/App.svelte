<script lang="ts">
  ...
  let nextCursor: string | null = null; let coverageNote = '';
  ...
  $: if (localMode && (localStatus === 'ready' || localStatus === 'loading') && remoteQuery !== lastRemoteQuery) { lastRemoteQuery = remoteQuery; const url = new URL(window.location.href); for (const key of ['country_code', 'category', 'q']) url.searchParams.delete(key); if (region !== 'all') url.searchParams.set('country_code', region); if (category !== 'all') url.searchParams.set('category', category); if (search.trim()) url.searchParams.set('q', search.trim()); history.pushState(null, '', url); void loadLocal(); }
  ...
  const syncRoute = async () => {
    ...
    else if (route.kind === 'home') { invalidateDetail(); selected = localMode ? loaded[0] : source[0]; }
  };
  const loadLocal = async () => {
    ...
    invalidateDetail(); lastRemoteQuery = `${profile}|${region}|${category}|${search}`;
    localStatus = 'loading'; localError = ''; manifestSha256 = undefined; loaded = []; selected = undefined; nextCursor = null; coverageNote = '';
    try { const result = await repo.list(profile === 'community' ? 'community' : 'official', { country_code: region === 'all' ? undefined : region, category: category === 'all' ? undefined : category }, controller.signal); if (generation !== listGeneration) return; loaded = result.locations; selected = result.locations[0]; release = result.releaseId; ruleset = result.ruleset; nextCursor = result.nextCursor; coverageNote = result.coverageNote; localStatus = 'ready'; await syncRoute(); }
    catch (error) { if (generation !== listGeneration) return; const kind = error && typeof error === 'object' && 'kind' in error ? (error as { kind: string }).kind : 'error'; localStatus = kind === 'no-release' ? 'no-release' : 'error'; localError = error instanceof Error ? error.message : 'Local V2 response was rejected safely.'; loaded = []; selected = undefined; }
  };
  ...
  onMount(() => {
    ...
    if (localMode) {
      search = params.get('q') ?? ''; region = params.get('country_code') ?? 'all'; category = params.get('category') ?? 'all';
    ...
    const onPopState = () => { const current = new URLSearchParams(window.location.search); if (localMode) { search = current.get('q') ?? ''; region = current.get('country_code') ?? 'all'; category = current.get('category') ?? 'all'; } onHashChange(); };
