...
describe('dev preview boundary', () => {
  ...
  it('preserves candidate unapproved and unpublished semantics', async () => {
    ...
    const unsafeBody = JSON.parse(JSON.stringify({ api_version: 'dev-preview-v1', data: [{ candidate_id: 'candidate-1', source_record_id: 'source-row-1', facility_id: 'facility-1', canonical_name: 'Candidate facility', country_code: 'DK', city: null, category: 'dairy', display_precision: 'unmapped', latitude: null, longitude: null, source_type: 'official', provenance_source_id: 'source-1', provenance_source_name: 'Private source', provenance_source_url: 'javascript:alert(1)', provenance_retrieved_at: '2026-01-01T00:00:00Z', factual_review_status: 'unreviewed', privacy_screening_status: 'passed', project_approval: false, release_id: 'candidate-release', release_status: 'candidate', preview_label: DEV_PREVIEW_LABEL }], meta: { test_only: true, private_preview: true, profile: null, coverage_scope: 'candidate_release_only', next_cursor: null } }));
    await expect(new DevCandidatePreviewRepository(vi.fn().mockResolvedValue(new Response(JSON.stringify(unsafeBody)))).list('operator-token')).rejects.toThrow('rejected safely');
  });
