...
test('completed France DGAL CLI run is API-readable and selectable at approximate commune locations', async ({ page }) => {
  ...
  const api = await page.evaluate(async (source) => {
    ...
    const viewport = await fetch(`/dev/real-preview/viewport?source_id=${encodeURIComponent(source)}&west=-6&south=41&east=11&north=52&limit=100`, { cache: 'no-store' }).then(response => response.json());
    const item = viewport.data.find((candidate: any) => candidate.display_precision === 'city_reference_approximate');
  ...
  expect(api.search.data.some((item: any) => item.candidate_id === api.item.candidate_id && item.display_precision === 'city_reference_approximate')).toBe(true);
